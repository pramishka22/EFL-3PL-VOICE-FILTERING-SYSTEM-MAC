        ...
        try:
            ...
            if ser.in_waiting:
                ...
                if "Distance:" in line:
                    ...
                    if len(parts) > 1:
                        latest_distance = parts[1].split("mm")[0].strip()
                        print(f"New measurement: {latest_distance}mm")
...
def get_length():
    try:
        return jsonify({
            'length': f"{float(latest_distance)/1000:.2f}",
            'status': 'active' if float(latest_distance) > 0 else 'idle'
        })
    except ValueError:
        return jsonify({'length': '0.00', 'status': 'error'})
